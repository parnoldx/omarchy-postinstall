...
# Remember the focused window so Insert path can paste back into it.
PREV_WIN_FILE="${XDG_RUNTIME_DIR:-/tmp}/fzfyazi-prev-window"
PREV_META="${XDG_RUNTIME_DIR:-/tmp}/fzfyazi-prev-window.meta"
_aw="$(hyprctl activewindow -j 2>/dev/null || true)"
_addr="$(printf '%s' "$_aw" | jq -r '.address // empty')"
_class="$(printf '%s' "$_aw" | jq -r '.class // empty')"
# If a previous finder is somehow focused, fall back to focus-history #1.
if [[ "$_class" =~ [Ff]inder|[Yy]azi ]] || [ -z "$_addr" ]; then
  _addr="$(hyprctl clients -j 2>/dev/null | jq -r 'sort_by(.focusHistoryID)[] | select((.class | test("finder|yazi"; "i") | not)) | .address' | head -1)"
  _class="$(hyprctl clients -j 2>/dev/null | jq -r --arg a "$_addr" '.[] | select(.address == $a) | .class' | head -1)"
fi
printf '%s' "$_addr" > "$PREV_WIN_FILE"
printf '%s\t%s\n' "$_addr" "$_class" > "$PREV_META" || true
