...
function labelWordStartsWith(label, needle) {
...
function searchScore(items, entry, query) {
  ...
  // "edg" in "Microsoft Edge" is a prefix of a name word, same as a whole
  // word ("zen" in Zen Browser). Treat that as an exact name hit so it
  // stays competitive among apps; the kind band below is what puts it
  // above Setup > Browser > Edge.
  else if (entry.kind === "app" && labelWordStartsWith(label, needle)) score = 0
  ...
  // Matching apps always list before menu/action rows. Prefix-labeled
  // setup/remove/channel items used to beat "Microsoft Edge" on "edg"
  // because they start with the query and the app only contains it.
  var kindRank = entry.kind === "app" ? 0 : 1
  return kindRank * 1000000 + score * 1000 + depthFor(items, entry.id) * 25 + (entry.order || 0)
}
...
// --- Time zones --------------------------------------------------------------
// "time in tokyo". QML's Date has no usable Intl timeZone option, so the zone
// list comes from timedatectl and each zone's current offset from `date`.
// Clock arithmetic is done here from that offset.
...
function isIanaZoneName(zone) {
  return /^[A-Za-z0-9_+\-\/]{1,64}$/.test(String(zone || ""))
}

// Exact zone or city beats a city that only starts with the query, so "york"
// does not win over "new york" and a half-typed name still finds something.
...
// "+0900" or "-0430" as seconds. null for anything else, so a failed `date`
// cannot be read as UTC.
