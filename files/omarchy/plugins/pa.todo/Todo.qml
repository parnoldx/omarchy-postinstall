...
Item {
  ...
  readonly property string pluginId: manifest?.id || "pa.todo"
  ...
  function submit() {
    ...
    // Same phrase grammar as the calendar widget's entry pane: a trailing
    // day, time, "!" priority or "/list" is pulled out; a plain line is
    // still just a title.
    var argv = Capture.todoAddArgv(captured, Date.now(), Model)
    if (!argv) return
    Quickshell.execDetached([Quickshell.env("HOME") + "/.local/bin/mailbox"].concat(argv))
  }
    ...
    BorderSurface {
      ...
      Item {
        ...
        Text {
          ...
          text: root.text || "Add a ToDo"
          // The text is text, never markup. Under the default AutoText a
