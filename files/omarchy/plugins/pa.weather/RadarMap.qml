...
Item {
  ...
  visible: shown
  // Collapse unless there is something to show: an actual echo near the user,
  // rain in the current conditions, or a confirmed "no coverage" notice.
  height: shown && (hasEcho || raining || coverageMissing) ? implicitHeight : 0
  clip: true
  ...
  // Windowed frame list: always starts one hour before "now"; extends one
  // hour after as well when short-range forecast frames exist (now in the
  // middle), otherwise ends at now. See RadarModel.windowFrames for the rules.
  readonly property var windowed: radarManifest ? RadarModel.windowFrames(radarManifest) : { frames: [], nowIndex: -1, now: 0 }
  readonly property var frames: windowed.frames
  readonly property int nowFrameIndex: windowed.nowIndex
  readonly property bool hasFutureFrames: !!(radarManifest && radarManifest.nowcast && radarManifest.nowcast.length > 0)
  readonly property string tileHost: radarManifest ? radarManifest.host : ""
  // Newest frame in the window. The echo probe samples this one to decide
  // whether there is actually rain to show, independent of playback position.
  readonly property var latestFrame: frames.length > 0 ? frames[frames.length - 1] : null
  // True once the newest frame shows an echo within ECHO_SHOW_RADIUS_KM of the
  // user. Gates the map's height so a dry, forecast-only day does not open an
  // empty radar. `raining` lets an actively-precipitating current condition
  // force the map open even before the probe has run.
  property bool hasEcho: false
  property bool echoChecked: false
  property string echoSummary: ""
  ...
  // Greyscale tile centred on the user, from the newest frame, scanned for any
  // echo. Greyscale (MEASUREMENT_SCHEME) is the channel analyzeEchoes measures
  // on; smooth/snow are off so reflectivity is read at face value.
  readonly property string echoProbeUrl: {
    if (!shown || !tileHost || !isFinite(latitude) || !isFinite(longitude) || !latestFrame) return ""
    return RadarModel.centeredTileUrl(tileHost, latestFrame.path, 256, RadarModel.MAX_RADAR_ZOOM, latitude, longitude, RadarModel.MEASUREMENT_SCHEME, false, false)
  }
  ...
  onShownChanged: {
    if (shown) {
      ...
      Qt.callLater(function() { echoProbe.probe() })
    } else {
      playing = false
    }
  }
  ...
  onFramesChanged: {
    ...
    if (frameA < 0) {
      // First population: open on "now" so it sits in the middle of the
      // windowed timeline; fall back to the latest frame when nothing lines
      // up with now.
      frameIndex = nowFrameIndex >= 0 ? nowFrameIndex : frames.length - 1
    } else if (frameIndex >= frames.length - 1 || frameIndex === 0) {
  ...
  Column {
    ...
    Item {
      ...
      Rectangle {
        ...
        // Scans the newest frame's greyscale tile, centred on the user, for any
        // echo within ECHO_SHOW_RADIUS_KM. Drives `hasEcho`, which collapses the
        // map on dry days so a forecast-only mention of rain does not open an
        // empty radar.
        Canvas {
          id: echoProbe
          width: 256
          ...
          property string probeUrl: root.echoProbeUrl
          ...
          onPaint: {
            ...
            ctx.drawImage(probeUrl, 0, 0, width, height)
            var pixels = ctx.getImageData(0, 0, width, height).data
            var echo = RadarModel.analyzeEchoes(pixels, width, root.latitude, RadarModel.MAX_RADAR_ZOOM, RadarModel.ECHO_SHOW_RADIUS_KM)
            root.hasEcho = echo.found
            root.echoSummary = RadarModel.summaryLabel(echo)
            root.echoChecked = true
          }
        }
    ...
    Item {
      ...
      Button {
        ...
        foreground: root.fg
        tooltipText: root.playing ? "Pause" : (root.hasFutureFrames ? "Play the last and next hour" : "Play the last hour")
      ...
      // Divider marking "now" within the windowed timeline. When future
      // frames are present it lands in the middle; otherwise it sits at the
      // right edge, where the history window ends.
      Rectangle {
        visible: root.frames.length > 1 && root.nowFrameIndex >= 0
        width: 2
        height: parent.height * 0.6
        anchors.verticalCenter: parent.verticalCenter
        x: timeline.x + (root.nowFrameIndex / Math.max(1, root.frames.length - 1)) * timeline.width - width / 2
        color: Color.accent
        opacity: 0.85
        radius: 1
      }
