...
Panel {
  ...
  // Indoor reading from Home Assistant. Kept on failure so a stale room
  // temperature stays visible rather than blinking empty on a dropped poll.
  property var roomTempState: null
  property var duskState: null
  property date now: new Date()
  readonly property string roomEntity: String(setting("roomEntity", "sensor.tvr_temperature") || "").replace(/^\s+|\s+$/g, "")
  readonly property string duskEntity: String(setting("duskEntity", "sensor.sun_next_dusk") || "").replace(/^\s+|\s+$/g, "")
  readonly property string reportRoomTemp: Model.formatHomeAssistantTemp(roomTempState, useImperial)
  readonly property string reportDuskIn: Model.formatCountdown(duskState, now)
  readonly property bool hasHomeRow: reportRoomTemp !== "" || reportDuskIn !== ""
  readonly property string haHelper: Quickshell.env("HOME") + "/.config/omarchy/plugins/pa.weather/ha-room-temp"
  ...
  function refreshRoomTemp() {
    if (roomTempProc.running || roomEntity === "") return
    roomTempProc.command = ["bash", root.haHelper, root.roomEntity]
    roomTempProc.running = true
  }

  function refreshDusk() {
    if (duskProc.running || duskEntity === "") return
    duskProc.command = ["bash", root.haHelper, root.duskEntity]
    duskProc.running = true
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var parsed = Model.parseHomeAssistantTemperature(text)
        if (parsed) root.roomTempState = parsed
      }
    }
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        var parsed = Model.parseHomeAssistantTimestamp(text)
        if (parsed) root.duskState = parsed
      }
    }
  }

  Timer {
    id: homeAssistantTimer
    interval: 60 * 1000
    running: root.roomEntity !== "" || root.duskEntity !== ""
    repeat: true
    triggeredOnStart: true
    onTriggered: root.refreshHomeAssistant()
  }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      RadarMap {
        ...
        raining: root.current ? Model.isPrecipitationPresent(root.current) : false
      }

      // ---- Indoor + dusk sit under the outdoor forecast. Room is a
      //      temperature; dusk is a countdown to next evening twilight.
      Rectangle {
        visible: root.hasHomeRow
        width: parent.width
        height: Style.spacing.hairline
        color: root.bar.foreground
        opacity: 0.12
      ...
      Item {
        ...
        Row {
          ...
          Row {
            visible: root.reportDuskIn !== ""
            spacing: Style.space(10)

            Text {
              anchors.verticalCenter: parent.verticalCenter
              text: "󰖛"  // nf-md-weather-sunset-down
              color: root.bar.foreground
              font.family: root.bar.fontFamily
              font.pixelSize: Style.font.display
            }
