...
// Window half-widths around the API's "now" (manifest.generated), in seconds.
// The window always starts one hour before now. With short-range forecast
// frames (nowcast) it extends one hour after now too, putting now in the
// middle; without future data it ends at now.
var CENTERED_HALF_WINDOW_SEC = 3600
...
// The frames the timeline actually shows, windowed around the API's notion of
// "now" (manifest.generated, which equals the latest past frame). Returns:
//   { frames: [...], nowIndex: int, now: epochSeconds }
// where `nowIndex` is the index of the frame closest to now within the window
// (the marker position), or -1 if the window is empty.
//
//   - With short-range forecast frames (nowcast) present: now sits in the
//     middle, one hour before and one hour after. This is the "now in the
//     middle" view the user asked for.
//   - Without future data: the trailing two hours of history, ending at now —
//     the plugin's original behaviour, preserved so a dry nowcast does not
//     shrink the view.
function windowFrames(manifest) {
  if (!manifest || !manifest.frames || manifest.frames.length === 0) {
    return { frames: [], nowIndex: -1, now: 0 }
  }
  var now = manifest.generated || manifest.frames[manifest.frames.length - 1].time
  var hasFuture = !!(manifest.nowcast && manifest.nowcast.length > 0)
  // Always start one hour before now; extend one hour after only when
  // short-range forecast frames are available, which centres now.
  var before = CENTERED_HALF_WINDOW_SEC
  var after = hasFuture ? CENTERED_HALF_WINDOW_SEC : 0
  var start = now - before
  var end = now + after
  ...
  for (var i = 0; i < manifest.frames.length; i++) {
    var time = manifest.frames[i].time
    if (time < start || time > end) continue
    windowed.push(manifest.frames[i])
    var delta = Math.abs(time - now)
    if (delta < bestDelta) {
      bestDelta = delta
      nowIndex = windowed.length - 1
    }
  }
  return { frames: windowed, nowIndex: nowIndex, now: now }
}
...
// How far around the user a radar frame must show an echo before the map is
// worth displaying. Wider than the alert radius on purpose: the map is opened
// so someone can watch a cell approach, not only when it is already overhead,
// but it has to be near enough to be relevant — a storm three countries away
// is not why the widget opened.
var ECHO_SHOW_RADIUS_KM = 30
