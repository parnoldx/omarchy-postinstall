...
Panel {
  ...
  // is skipped.
  readonly property var dailyChartHiddenProviderIds: ["claude", "grok", "hyper"]
  ...
  KeyboardPanel {
    ...
    // Tall enough that the whole dashboard fits without scrolling: the cap is
    // the space under the bar (fittedContentHeight clamps to it anyway), not an
    // arbitrary 760 that made three stacked subscriptions scroll.
    contentHeight: panel.fittedContentHeight(column.implicitHeight, panel.availableCardHeight)
