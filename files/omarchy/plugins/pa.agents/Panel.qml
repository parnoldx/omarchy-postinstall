...
Panel {
  ...
  // Personal display preference, not a data-quality omission (unlike Grok's
  // collector, which never fills recentDays at all): Claude's daily chart is
  // real data, just not something this panel shows. The underlying record
  // keeps recentDays intact for sync/other consumers — only the section
  // below is skipped.
  readonly property var dailyChartHiddenProviderIds: ["claude"]
