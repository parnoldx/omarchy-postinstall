...
#
# Included: Hyprland overlays, NeoQwertz keymap, bar plugins, Voxtype dictation
# (Ctrl+F1 insert / Ctrl+Shift+F1 process / Ctrl+F2 ask / Ctrl+Shift+F2 do,
# CPU Parakeet via the voxtype-bin AUR package), WhatsApp (wacli mirror +
# whatsapp-gui blade), Chromium/Edge video decode on the Intel iGPU (Omarchy's
# NVIDIA VAAPI freezes HTML5 players on this hybrid laptop), Home Assistant
# helpers, Grok/Hyper usage collectors, Grok theme sync, branding, Plymouth/SDDM
# Om unlock, default agent `pi`, VS Code as the editor (Omarchy launcher and
# Files), a Super+Ctrl+F fzf+yazi file finder, a `kill-tui` process killer,
# community bar plugins (installed via `omarchy plugin add`), omatts TTS,
# omavoice meeting export, slitherlink + sudoku puzzles, and extra packages.
#
...
# go is for building mailbox from source; dropbox backs the omarchy.dropbox
# bar widget.
REPO_PACKAGES=(thunderbird bitwarden bitwarden-cli git-lfs libqalculate go rbw fd fzf yazi flea jq wtype wl-clipboard dropbox)
AUR_PACKAGES=(cadassistant-appimage voxtype-bin wacli microsoft-edge-stable-bin visual-studio-code-bin)
...
THIRD_PARTY_PLUGINS=(
  "37signals.basecamp|https://github.com/basecamp/omarchy-basecamp-plugin.git"
  "andreconde.quick-look|https://github.com/andreconde21/omarchy-quick-look.git"
  "firstpick.keybindings|https://github.com/Firstp1ck/omarchy-keybindings"
  "gigasolo.omavoice|https://github.com/gigasolo/omavoice.git"
...
# Own GitHub repos cloned and built locally: dir|repo-url|install-command
OWN_REPOS=(
  "whatsapp-gui|https://github.com/parnoldx/whatsapp-gui.git|make dependencies install"
  "omaslitherlink|https://github.com/parnoldx/omaslitherlink.git|./install.sh"
  "omasudoku|https://github.com/parnoldx/omasudoku.git|./install.sh"
)
...
desktop:
  ...
  Hyprland    scale 1.25, gaps 2/4, rounding 4, NeoQwertz keymap,
              Thunderbird overlay, Voxtype dictation (CPU Parakeet),
              Super+J layout toggle, Chromium/Edge VAAPI on Intel iGPU
  Bar         transparent; pa.menu / mailbox.clock / mailbox.email /
              pa.weather / pa.tray / pa.agents / Voxtype / pa.chord-expander /
              pa.todo / Omavoice / YouTube Music / Time Machine, Dropbox,
              Omatree (grouped into Pocket); keyboard-layout; firstpick
              keybindings editor
  Finder      Super+Ctrl+F fzf+yazi file search (also browse / by-type
              variants); Super+Shift+F flea file manager
  Apps        whatsapp-gui (own repo, right-edge blade), omatts TTS,
              slitherlink + sudoku (own repos), omavoice meeting export,
              Windows VM prep wrapper
  Plugins     Quick Look, Time Machine, Pocket, Omatree, YouTube Music,
              Basecamp, keybindings editor, Omavoice (community, via
...
for f in hyprland.lua bindings.lua autostart.lua input.lua looknfeel.lua monitors.lua \
         hyprsunset.conf xdph.conf omarchy-workspace-layout.lua; do
...
chmod 755 "$HOME/.config/omarchy/plugins/pa.weather/ha-room-temp" \
  "$HOME/.config/omarchy/plugins/pa.dictation-answer/toast-offset" \
  "$HOME/.config/omarchy/bar/scripts/voxtype-status" \
  "$HOME/.config/omarchy/bar/scripts/voxtype-levels"
...
# GUI message viewer the email panel shells out to when present (optional,
# but build it so the panel works fully out of the box).
if make -C "$mailbox_src" -n install-gui >/dev/null 2>&1; then
  run make -C "$mailbox_src" install-gui
else
  warn "mailbox install-gui target not found; email panel falls back to the TUI"
fi
...
install_file "$FILES/omarchy/workspace-layout.json" \
  "$HOME/.config/omarchy/workspace-layout.json"
...
# Grok theme sync: a themed/ alacritty template + wrappers that patch the
# packaged omarchy-theme-set so Grok's terminal colors follow the bar theme.
log "Installing Grok theme tooling"
install_tree "$FILES/omarchy/themed" "$HOME/.config/omarchy/themed"
install_tree "$FILES/omarchy/bin" "$HOME/.config/omarchy/bin"
chmod 755 "$HOME/.config/omarchy/bin/omarchy-theme-set-grok" \
  "$HOME/.config/omarchy/bin/patch-grok-omarchy-theme"
...
for f in voxtype-toggle voxtype-dictation voxtype-agent-toggle voxtype-ask-toggle \
         voxtype-do-toggle voxtype-fix voxtype-learn; do
  install_file "$FILES/bin/$f" "$HOME/.local/bin/$f" 755
done
...
install_file "$FILES/bin/omarchy-agent-usage-hyper" "$HOME/.local/bin/omarchy-agent-usage-hyper" 755
install_file "$FILES/bin/omarchy-resume-guard" "$HOME/.local/bin/omarchy-resume-guard" 755
install_file "$FILES/bin/omavoice-meeting-export" "$HOME/.local/bin/omavoice-meeting-export" 755
install_file "$FILES/bin/chord-expander" "$HOME/.local/bin/chord-expander" 755
...
# Clears suid/sgid on the VM dirs before Omarchy's exact-mode 0700 check,
# which otherwise aborts launch when a Nautilus/archive copy brought setgid
# along. The desktop entry below calls this instead of the packaged binary.
install_file "$FILES/bin/omarchy-windows-vm-prep" \
  "$HOME/.local/bin/omarchy-windows-vm-prep" 755
# Chrome native-messaging biometric dialog used by vault-bridge.
mkdir -p "$HOME/.local/bin/vault-bridge-dialog"
install_file "$FILES/bin/vault-bridge-dialog/shell.qml" \
  "$HOME/.local/bin/vault-bridge-dialog/shell.qml"

# voxtype dictation config (model + correction list). Existing config wins.
if [[ ! -f $HOME/.config/voxtype/config.toml ]]; then
  mkdir -p "$HOME/.config/voxtype"
  cp "$FILES/voxtype/config.toml" "$HOME/.config/voxtype/config.toml"
  cp "$FILES/voxtype/corrections.tsv" "$HOME/.config/voxtype/corrections.tsv"
fi

# --- own GitHub repos ---------------------------------------------------------

# whatsapp-gui (Qt Quick blade client, reads the wacli mirror), slitherlink +
# sudoku (native puzzle games), omatts (local TTS, curl installer).
log "Installing own GitHub repos"
for entry in "${OWN_REPOS[@]}"; do
  IFS='|' read -r dir repo cmd <<<"$entry"
  src="$HOME/.local/src/$dir"
  if [[ ! -d $src/.git ]]; then
    run git clone --depth 1 "$repo" "$src"
  fi
  run bash -c "cd '$src' && $cmd"
done
if [[ ! -x $HOME/.local/bin/omatts ]]; then
  run bash -c "curl -fsSL https://raw.githubusercontent.com/parnoldx/omatts/master/install.sh | OMATTS_PACKS=de sh"
fi
...
# Distinguishes the WhatsApp Web launcher from the local whatsapp-gui app.
install_file "$FILES/applications/WhatsApp.desktop" \
  "$HOME/.local/share/applications/WhatsApp.desktop"
# CAD Assistant local override: same xcb fix as the /usr/share sed below, but
# this one shadows the packaged file and survives omarchy updates.
install_file "$FILES/applications/cadassistant.desktop" \
  "$HOME/.local/share/applications/cadassistant.desktop"
# Git identity + gh credential helpers (safe.directory entries stay
# machine-local; add them per checkout if needed).
install_file "$FILES/git/config" "$HOME/.config/git/config"
# mpv: RTSP over UDP + uosc overlay controls, autohide like elementary.
install_file "$FILES/mpv/mpv.conf" "$HOME/.config/mpv/mpv.conf"
# gcr ssh-agent socket + en_GB time format for the session.
install_tree "$FILES/environment.d" "$HOME/.config/environment.d"
# Windows VM launcher via the suid/sgid-clearing wrapper (see bin above).
install_file "$FILES/applications/windows-vm.desktop" \
  "$HOME/.local/share/applications/windows-vm.desktop"
# Wayland + VAAPI-on-Intel flags for Chromium and Edge (Omarchy's NVIDIA VAAPI
# freezes HTML5 video on this hybrid laptop).
install_file "$FILES/chromium-flags.conf" "$HOME/.config/chromium-flags.conf"
install_file "$FILES/microsoft-edge-stable-flags.conf" \
...
run omarchy hook install theme-set "$FILES/omarchy/hooks/theme-set.d/sync-grok-theme.hook"
# One-time invitations after omarchy updates: fingerprint reader and default
# agent setup. Both no-op once their setup is done.
run omarchy hook install post-update "$FILES/omarchy/hooks/post-update.d/setup-fingerprint.hook"
run omarchy hook install post-update "$FILES/omarchy/hooks/post-update.d/setup-agent.hook"
...
if command -v systemctl >/dev/null; then
  ...
  # Handy stack is gone; voxtype replaced it.
  systemctl --user disable --now handy-gpu-keepalive.service 2>/dev/null || true
  rm -f "$HOME/.config/systemd/user/handy-gpu-keepalive.service"
  for bin in handy-daemon handy-toggle handy-agent-toggle handy-ask-toggle \
             handy-do-toggle handy-paste handy-dictation handy-gpu-keepalive \
             handy-ensure-settings; do
    rm -f "$HOME/.local/bin/$bin"
  done
  systemctl --user daemon-reload
  for unit in omarchy-agent-usage-grok.timer omarchy-agent-usage-grok.path \
              omarchy-agent-usage-hyper.timer voxtype.service \
              omarchy-resume-guard.service hyprsunset-solar.timer \
              hyprsunset-solar-sunrise.timer hyprsunset-solar-sunset.timer \
              hyprsunset-solar-resume.service; do
    systemctl --user enable --now "$unit" || warn "could not enable $unit"
  done
  if command -v wacli >/dev/null; then
    systemctl --user enable --now wacli-sync.service || warn "could not enable wacli-sync.service"
  fi
  if [[ -f $mailbox_config ]]; then
  ...
  systemctl --user start omarchy-agent-usage-hyper.service ||
    warn "Hyper usage collector failed (run hyper login later)"
...
if command -v hyprctl >/dev/null && hyprctl version >/dev/null 2>&1; then
  # chord-expander writes its submap once and appends an include line to
  # bindings.lua; snippet edits never need a reload afterwards.
  if [[ -x $HOME/.local/bin/chord-expander ]] && ! grep -q "chord-expander/submap.lua" "$HOME/.config/hypr/bindings.lua"; then
    "$HOME/.local/bin/chord-expander" install || warn "chord-expander install failed; run it later"
  fi
...
log "Voxtype dictation: Ctrl+F1 insert, Ctrl+Shift+F1 process, Ctrl+F2 ask, Ctrl+Shift+F2 do; Ctrl+F3 learns a correction."
log "File transcription: transcribe file.mp4 [-o out.txt]."
log "Thunderbird overlay: Super+Shift+E."
log "Mailbox panel: Super+Shift+Alt+E. Calendar: Super+Ctrl+D."
log "File finder: Super+Ctrl+F. File manager: Super+Shift+F. Kill a process: menu -> Kill Process."
