...
`chord-expander` (Super+E, then a 1–5 char trigger; leader twice opens the picker) is installed and registered into `bindings.lua` on first run; snippets live in `~/.config/chord-expander/snippets.json` and hot-apply without a reload.

Super+Ctrl+F fuzzy-finds files with fzf + yazi in a floating terminal (Super+Ctrl+Shift+F browses, Super+Ctrl+Alt+F searches by type); Super+Shift+F/Super+Alt+Shift+F open `flea`, a graphical file manager. `kill-tui` (Super menu → Kill Process) fuzzy-finds a running process and sends SIGTERM/SIGKILL.
